...
/**
 * Soft-dependency bridge to SDM Economy ("sdmeconomy"), the default currency for the Buy Claims button.
 * {@link #isLoaded()} touches no SDM type, so the class loads fine when SDM is absent; the SDM-typed methods
 * below are only ever called after isLoaded() returns true, so their classes resolve lazily and never crash
 * a server without SDM installed. (Mirror of the efwarborn SdmBridge used by /factionbuy.)
 */
